...
const SPLITS: { id: SplitType; label: string; hint: string }[] = [
  ...
  { id: "exclusive", label: "Individual", hint: "Só uma pessoa consumiu isso" },
  { id: "custom", label: "Personalizado", hint: "Você define valor ou % por pessoa" },
];

const CUSTOM_MODES: { id: CustomMode; label: string }[] = [
...
export function ExpenseEditor({
  ...
  const percentSum = useMemo(
    () => draft?.percentages.reduce((a, p) => a + p.percent, 0) ?? 0,
    [draft],
  );
  ...
  if (draft.splitType === "exclusive" && draft.sharedWith.length !== 1)
    errors.push("Escolha quem consumiu sozinho.");
  if (draft.splitType === "custom" && diff !== 0)
    errors.push(
      diff > 0
  ...
  const balanced = draft.splitType === "custom" && draft.totalAmount > 0 && diff === 0;
    ...
    <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
      <DialogContent className="max-h-[92vh] w-[calc(100%-1.5rem)] max-w-lg overflow-y-auto rounded-3xl p-0 sm:max-w-lg">
        ...
        <div className="space-y-6 px-5 py-5">
                ...
                <button
                  ...
                  onClick={() =>
                    patch({
                      splitType: s.id,
                      sharedWith:
                        s.id === "exclusive" && draft.sharedWith.length !== 1
                          ? [draft.sharedWith[0] ?? draft.paidBy]
                          : draft.sharedWith,
                      allocations:
                        s.id === "custom" &&
                        draft.customMode === "amount" &&
          ...
          {draft.splitType === "exclusive" ? (
            <div className="space-y-2">
              <p className="text-sm font-semibold">Quem consumiu sozinho?</p>
              <div className="flex flex-wrap gap-2">
                {participants.map((p) => (
                  <ParticipantChip
                    key={p.id}
                    id={p.id}
                    name={p.name}
                    selected={draft.sharedWith[0] === p.id}
                    onClick={() => patch({ sharedWith: [p.id] })}
                  />
                ))}
              </div>
            </div>
          ...
          {draft.splitType === "custom" ? (
            <div className="space-y-3">
                ...
                <div className="flex gap-1 rounded-2xl border border-border bg-muted/40 p-1">
                  {CUSTOM_MODES.map((m) => (
                    <button
                      key={m.id}
                      type="button"
                      onClick={() => patch({ customMode: m.id })}
                      aria-pressed={draft.customMode === m.id}
                      className={cn(
                        "rounded-xl px-3 py-1.5 text-xs font-semibold transition-colors",
                        draft.customMode === m.id
              ...
              {draft.customMode === "amount"
                ...
                : participants.map((p) => {
                    const value =
                      draft.percentages.find((w) => w.participantId === p.id)?.percent ?? 0;
                    return (
                      <div key={p.id} className="flex items-center gap-3">
                        <ParticipantAvatar id={p.id} name={p.name} size="sm" />
                        <span className="flex-1 truncate text-sm font-medium">{p.name}</span>
                        <div className="relative w-24">
                          <input
                            aria-label={`Porcentagem de ${p.name}`}
                            type="number"
                            min={0}
                            max={100}
                            value={value || ""}
                            onChange={(e) =>
                              patch({
                                percentages: [
                                  ...draft.percentages.filter((w) => w.participantId !== p.id),
                                  {
                                    participantId: p.id,
                            ...
                            className="money w-full rounded-2xl border border-input bg-card py-2 pr-7 pl-3 text-right font-bold"
                          />
                          <span
                            aria-hidden="true"
                            className="pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 text-sm font-semibold text-muted-foreground"
                          >
                            %
                          </span>
                        </div>
          ...
          {draft.splitType === "custom" ? (
            ...
            >
              ...
              {draft.customMode === "percentage" ? (
                <span>{percentSum}% / 100%</span>
              ) : (
