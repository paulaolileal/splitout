...
| `src/hooks/queries.ts`                                | TanStack Query hooks: `useParties`, `useParty` (returns `{ party, update }`), `useCreateParty`, `useDeleteParty`, `usePeople`, `useSavePerson`, `useDeletePerson`. `useParty(id)` deliberately reads the _same_ `qk.parties` query as `useParties()` (via `select`) instead of its own per-id key — a party detail read is the same `readAllRows()` over the same six tabs as the list read, so sharing the cache entry means opening several rolês in one session only pays for one network read, not one per rolê (see "Google Sheets API usage" below) |
...
| `src/presentation/icons/registry.tsx`                 | Catalogue of `@icon-park/react` "multi-color" icons that back `Party.emoji`/`Expense.emoji`: `FOOD_ICONS`/`SPORT_ICONS`/`ACTIVITY_ICONS` (grouped under `ICON_CATEGORIES`), a `PALETTE` of named color tuples, and `AppIcon` — the only place that resolves a stored key to its icon component + `PALETTE` fill and renders it; unknown keys fall back to the default expense icon instead of rendering as text. See "Icons" below                                                                                                                        |
...
| `src/presentation/components/DatePicker.tsx`          | Popover+`Calendar` (shadcn/react-day-picker, pt-BR locale) date field used by `NewPartyPage`/`PartyEditor` — replaced the native `<input type="date">`, whose calendar glyph and dropdown are drawn by the browser/OS and ignore the app's dark theme. Value/onChange stay plain ISO `yyyy-MM-dd` strings                                                                                                                                                                                                                                                 |
...
| `src/services/googleAuth.ts`                          | OAuth via the shared `lealtek-api` backend (Authorization Code + PKCE — see the `lealtek-api` repo); access token lives **in memory + sessionStorage only** — never localStorage, and there is no client-side client_id/secret. `signIn()` is a full-page redirect to `lealtek-api`'s login endpoint; `initAuthScheduler()` (called once in `main.tsx`) consumes the token left in the URL after that redirect, then proactively renews it before it expires and on tab focus/visibility (`POST /api/auth/refresh`, reading an httpOnly cookie server-side) |
...
| `src/infrastructure/google/googleApiFetch.ts`         | Shared fetch wrapper used by all Google REST clients; ensures a fresh token per call and throws `GoogleAuthError` when silent refresh genuinely fails, or `GoogleQuotaError` on an HTTP 429 (per-user Sheets/Drive rate quota exceeded)                                                                                                                                                                                                                                                                                                                   |
...
| `src/lib/googleQuotaToast.ts`                         | Transient toast shown whenever a `GoogleQuotaError` bubbles up through React Query's cache (Sheets/Drive rate quota exceeded) — both this and `googleAuthToast.ts` are wired once in `main.tsx`'s shared `QueryCache`/`MutationCache` `onError`, so no individual query/mutation needs its own `onError`. `main.tsx`'s global `retry` function also skips retrying either error — retrying a quota error immediately just doubles the read volume right when the quota is already exceeded                                                                |
...
`ICON_CATEGORIES` for `IconPicker`, and the `<AppIcon iconKey={...} />`
component to go from a stored key back to a rendered icon anywhere it's
used (`HomePage`, `PartyPage`, `SamplePage`, `cards.tsx`'s `ExpenseCard`,
`IconPicker` itself). icon-park's "multi-color" theme uses one fixed
palette for every icon unless a `fill` array is passed, so `AppIcon` also
looks up that icon's `PALETTE` tuple (`[outline, main, innerHighlight,
innerFill]` — named hues like `PALETTE.yellow`/`PALETTE.red`, picked per
icon's subject) and passes it as `fill`; this is what makes each icon read
as its own colorful "sticker" instead of a uniform blue tint. Always
render a catalogue icon through `AppIcon` rather than pulling `Icon`/`fill`
off an `IconOption` by hand — that pairing is the whole point of the
component.
...
emoji in the `emoji` column just fall back to `AppIcon`'s generic default
instead of rendering correctly; there was no need to migrate existing
data.
...
1. Reads all six party tabs in one `values:batchGet` call (`readAllRows`) —
   each Sheets API call counts once against the per-user read-quota, so
   batching the six tabs into one request instead of six separate
   `values.get` calls matters for staying under it. The response's
   `valueRanges` are matched back to a tab by **array position**, not by the
   echoed `ValueRange.range` string: the Sheets API normalizes that field to
   the data's actual extent (e.g. `"parties!A1:F2"`, never the bare sheet
   name requested), but always returns `valueRanges` in the same order the
   `ranges` were requested — matching by position sidesteps the mismatch
   entirely.
...
2. Criar esse registro no provedor DNS de `lealtek.com`. Se estiver atrás do
   ...
   SSL da Vercel.
3. **Crítico**: no projeto `lealtek-api` (backend compartilhado de auth), a
   env var `ALLOWED_ORIGINS` precisa incluir `https://split.lealtek.com` —
   é essa allowlist que autoriza o CORS de `/api/auth/refresh` e valida o
   `return_to` de `/api/auth/login`. Sem isso o login falha (ou o refresh
   silencioso fica bloqueado por CORS) nesse domínio. O Google Cloud Console
   em si não precisa mais conhecer `split.lealtek.com` — o app não fala mais
   diretamente com o Google, só com `api.lealtek.com`.
